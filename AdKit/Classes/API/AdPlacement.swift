...
public protocol NativeAdPlacement {
    ...
    /// Баннерное место для того же экрана. Нужно там, где у медиатора нет нативки
    /// (сейчас это Яндекс) и вместо неё показывается обычный баннер.
    /// `nil` — фолбэка нет, место просто останется пустым.
    var bannerFallback: AdPlacement? { get }
}
