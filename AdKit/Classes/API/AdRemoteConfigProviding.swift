...
public protocol AdRemoteConfigProviding: AnyObject {
    ...
    /// Значения конфига уже доступны: либо фетч удался в этом запуске, либо
    /// лежат в кэше от прошлого. На самой первой установке — `false`, и пакет
    /// в этот момент не принимает решение «рекламы нет», а ждёт сигнала.
    var isConfigAvailable: Bool { get }
